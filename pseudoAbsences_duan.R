...
pseudoAbsences_duan <- function (xy, background, realizations = 1, exclusion.buffer = 0.0166, prevalence = 0.5, 
...
pseudoAbsences0 <- function(xy, background, exclusion.buffer = 0.0166, prevalence = 0.5, 
  ...
  for(j in 1:10){
    for (i in 1:nrow(pr)) {
      discbuff <- disc(radius = r, centre = c(pr[i,1], pr[i,2]))
    ...
    aus <- list()
    coords.d <- list() 
    coords.d[[1]] <-dplyr::setdiff(coords.l[[1]], aa[[1]])
    for (i in 1:length(coords.d)) {
...
bindPresAbs <- function (presences, absences){
  ...
  for (j in 1:length(au)){
    aj <- cbind(as.data.frame(au[[j]]), rep(0,length(au[[j]])))
    names(aj)<-names(pr)
    prau[[j]]<-rbind(pr, aj)
  }
  names(prau)<-names(au)
